# Basic Terminal and Navigating the Filesystem
...
If you open the file `.bashrc` using the command `subl ~/.bashrc`, you'll see your own config file for bash. Notice that yours references `.bash_profile`. Let's open that using `subl .bash_profile`.
...
You can add folders in the `$PATH` by adding them in the bash (or zsh) config file. If you open your terminal config file, you’ll see a section looking like:
...
We will use the command line several hours every day, because it makes all file and folder manipulations easier. A lot of softwares programs that we will use during the course also only have a CLI interface and can only be used with commands. Always remember that every action you'll do in a GUI can be done in the CLI.
